...
const Model = () => {
    ...
    const handleActive = async (id) => {
        await axios.get(`https://httm1-production.up.railway.app/api/bien/activeModel/${id}`);
        setUpdate(!update);
    };
    ...
    return (
        <div className="mt-[40px]">
            <p className="text-[30px] font-bold mb-[40px]">Quản lý model biển</p>
            <div className="flex justify-between px-[140px] mb-[40px]">
                <div className="flex">
                    <Link to={`/addModel/bien`}>
                        ...
                        {modelShow.length !== 0 &&
                            modelShow.map((row) => (
                                <TableRow key={row.name} sx={{ "&:last-child td, &:last-child th": { border: 0 } }}>
                                    ...
                                    <TableCell align="right" style={{ textAlign: "left" }}>
                                        <Switch checked={row.active} onChange={() => handleActive(row.id)} />
                                    </TableCell>
